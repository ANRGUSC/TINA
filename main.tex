...
This section verifies the closed-form laws and illustrates the architectural
shifts they predict on finite graphs.
...
agentwise Gaussian conditional expectations are team-optimal.
...
The experiments are consistency checks and finite-graph illustrations within
the common-rate Gaussian-affine model; robustness beyond that model is
discussed in Section~\ref{sec:discussion_conclusions}. Exact system matrices,
graph seeds, conditioning thresholds,
...
\midrule
1. Temporal law & $\tau/T\in[0,3]$ & $n=8,20,32,50$ & $20{,}000$/point \\
2. Local--global & $\gamma/q\in[0,10]$ & $n=5,10,25,100$ & $30{,}000$/setting \\
3. Radius curves & $r/\ell_c\in[0,2]$ & three regimes & $50{,}000$/point \\
4. Radius phase & $vT/\ell_c\in[0.1,20]$ & $72\times68$ grid & covariance \\
5. Decision relevance & $\ell_c=0.7,2,6$ & 64-node ring & $12{,}000$/check \\
6. General graphs & path, ring, grid, geometric & 64 nodes & covariance \\
\bottomrule
\end{tabular}
\end{table}
...
The first two experiments test the temporal law
\eqref{eq:OU_exact_regret} and the crossover threshold
\eqref{eq:aggregate_rho_star}. For the temporal law, we used four systems
spanning different dimensions, spatial covariances, objective matrices, and
decision operators. Across 100 parameter points, the Monte Carlo curves
collapse onto $1-e^{-2\tau/T}$, with a largest absolute normalized deviation
of $0.0155$ (Fig.~\ref{fig:temporal-scaling}).
...
$(n,\gamma/q)$ settings, the mean absolute boundary error was $0.00111$.
Finite size matters most under strong coupling:
...
This experiment tests the marginal-balance condition
\eqref{eq:marginal_balance_condition} by decomposing regret into the temporal
and spatial terms of Eq.~\eqref{eq:R_decomposition}. Using
$\eta_S(r)=\eta_0e^{-2r/\ell_c}$ and $\tau(r)=r/v$,
Fig.~\ref{fig:radius-regret} exhibits local, interior, and boundary (global)
regimes on $0\le r/\ell_c\le2$. The grid and analytical minimizers agree to
within grid resolution in all three regimes. At the interior optimum,
$|m_S(\hat r^\star)-m_T(\hat r^\star)|=4.77\times10^{-4}$, directly confirming
the marginal-balance prediction.
...
the predicted zero-to-positive-radius threshold at $vT=2\ell_s$ and agreement
with Eq.~\eqref{eq:canonical_dimensionless_law} to within grid resolution;
exact figures are reported in the reproduction package. Discrepancies occurred
only within one grid step of the transition. Separate
one-dimensional numerical sweeps were
...
correlation was identical, so changing only which spatial modes affect the
desired decision produced substantially different architectures.
...
$0.7$ to $6$ moved the optima from at most one hop to as many as five.
Increasing per-hop latency weakly decreased every optimum.
...
differences, the discrete analogue of marginal balance.
