...
\hypersetup{pdftitle={Too Late to Coordinate: When Local Information Beats Global Sharing},pdfauthor={Scott Moeller and Bhaskar Krishnamachari},pdfsubject={IEEE Control Systems Letters manuscript},pdfkeywords={Decentralized control, Sensor fusion, Control system architecture}}
...
\title{Too Late to Coordinate:\\When Local Information Beats Global Sharing}
...
\pagestyle{empty}
\begin{abstract}
Networked sensor fusion must balance the benefit of shared measurements against the delay and cost of communication. We study this tradeoff for a Gaussian team tracking a changing signal with a penalty for disagreement, assuming a common temporal decay rate for the signal and sensor errors. An exact delay threshold identifies when fresh local decisions outperform decisions based only on a complete delayed pool. When agents retain fresh local observations and their local sample from the pooling time, one shared scalar achieves the same performance as full delayed sharing. The resulting benefit over local operation decays exponentially at twice the signal's correlation decay rate. Integrating that benefit gives the maximum update price for which refreshing can improve long-run average cost at a given communication latency. At or above this price, no refreshing is optimal. Below it, periodic refreshing has a unique optimal interval and is optimal among all schedules independent of the sensor processes. The pooling law extends to unequal sensor quality and independent components with different temporal rates.
\end{abstract}
\noindent\textbf{Index Terms---}Decentralized control, sensor fusion, control system architecture.

\section{Introduction}
In networked sensor fusion, agents share measurements to improve the accuracy and consistency of their decisions. Collecting and distributing those measurements takes time. With enough delay, using all sensors' data can yield worse decisions than acting on fresh local measurements. We identify the exact delay beyond which this occurs in a Gaussian sensor model, optimizing both decision rules for their available information.

We consider agents tracking a common changing signal through noisy local sensors. Their shared objective penalizes tracking error and disagreement among their actions. Agents relying only on the delayed pool agree perfectly. Beyond the delay threshold, their tracking error outweighs this agreement benefit, and fresh local decisions achieve lower total loss. The signal and sensor errors evolve as Gaussian processes with a common temporal decay rate $\lambda$. Agents can use fresh local information, a delayed pool, or a hybrid of both. Actions do not affect future observations, so each decision is a static team problem.

Our main results are the following:
\begin{enumerate}
\item \emph{Optimal decisions and a delay threshold.} We derive explicit fresh-local and fresh-pooled policies, posterior variances, and costs, and evaluate the local-versus-delayed-global crossover for this sensor model (Proposition~\ref{prop:endpoints}, Corollary~\ref{cor:crossover}).
\item \emph{Hybrid sharing and exact decay.} With fresh local observations and the local sample from the pooling time retained, a single shared scalar achieves the same optimal team loss as sharing every sensor's delayed measurements. We derive the optimal policy and posterior variance. The improvement over local operation decays as $e^{-2\lambda\tau}$ (Theorem~\ref{thm:hybrid}).
...
\item \emph{When and how often to refresh.} Each pooled update provides a positive benefit at every finite age. Its total benefit over time is finite and determines the critical update price for a given communication latency. We characterize when no refreshing is optimal and prove that below the critical price, periodic refreshing has a unique optimal interval and is optimal among all schedules independent of the observations (Theorem~\ref{thm:refresh}).
\end{enumerate}
...
Kar and Varshney study sensor collaboration and sampling of an Ornstein--Uhlenbeck signal under a power budget \cite{kar2012controlled}. Nayyar, Ba\c{s}ar, Teneketzis, and Veeravalli jointly optimize communication and remote estimation under communication costs and energy constraints \cite{nayyar2013optimal}. Their transmission decisions can depend on sensor observations. Our setting gives every agent fresh private observations between pooled updates and penalizes disagreement among decisions. Our refresh schedules are independent of observed values.

The broader study \cite{tina2026} establishes the general freshness--locality crossover and develops spatial predictability and neighborhood-radius selection. This letter specializes that crossover to a common-signal Gaussian sensor team. We extend that work with the exact hybrid value and a proof that one shared scalar achieves the value of complete delayed sharing. Further results cover unequal sensor variances and independent components with different rates, and characterize optimal refresh schedules.
...
\begin{table}[t]
\vspace*{7pt} % Keep the top caption inside the submission margin.
\caption{Data sufficient to implement the optimal policies.}
...
The general freshness--locality crossover in \cite{tina2026} compares current local information with complete delayed global information. For this sensor model, it takes the following explicit form.
...
\begin{figure}[t]
 \vspace*{7pt} % Keep figure paths inside the submission margin.
 \centering
 \includegraphics[width=\columnwidth]{figures/pooling_decay.pdf}
 \caption{The delay crossover and hybrid value. (a) Expected team loss under four information architectures. Beyond the crossover age, fresh-local operation outperforms the delayed pool used alone. The hybrid achieves lower loss than fresh-local operation at every finite age. (b) The hybrid's improvement over fresh-local operation, normalized by the improvement from fresh pooling, equals $e^{-2\lambda\tau}$ across the illustrated parameter settings. Markers show Monte Carlo estimates.}
 \label{fig:decay}
\end{figure}
Figure~\ref{fig:decay} uses 120,000 independent stationary Gaussian sample pairs per marker. We also solve the Gaussian team normal equations directly from observation covariances. Across 40 random parameter sets with $2\le n\le12$, $0.1\le a,b\le10$, $0.01\le\kappa\le10^{1.5}$, and $0.01\le\rho\le0.98$, the local and hybrid architectures give 80 systems. Three additional hybrid cases with $\kappa=0$ check $\rho\in\{0,0.5,0.999\}$. The maximum cost discrepancy across all 83 systems is below $2.7\times10^{-15}$.
...
\begin{figure}[t]
 ...
 \caption{Average team loss plus communication cost for the hybrid architecture with periodic pooled updates and continued access to fresh local observations. The dashed line shows fresh-local operation without updates. Below the critical update price, each cost curve has a unique finite minimizer. At or above that price, no refreshing is optimal.}
 \label{fig:refresh}
\end{figure}
...
Generative AI (OpenAI ChatGPT) was used to assist with drafting and copy-editing the paper, mathematical proofs and numerical simulations. All technical claims and proofs have been verified. The authors take full responsibility for the manuscript.
\end{samepage}
