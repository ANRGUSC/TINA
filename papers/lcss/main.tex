...
\hypersetup{pdftitle={How Fast Does Coordination Value Decay? Exact Delayed-Pooling Laws for Gaussian Teams},pdfauthor={Scott Moeller and Bhaskar Krishnamachari},pdfsubject={Self-contained first draft for IEEE Control Systems Letters},pdfkeywords={Decentralized control, Sensor fusion, Control system architecture}}
...
\title{How Fast Does Coordination Value Decay?\\Exact Delayed-Pooling Laws for Gaussian Teams}
...
\pagestyle{empty}
\begin{abstract}
Fresh local measurements can yield better team decisions than a delayed pool containing every sensor's data. We derive the exact delay at which this reversal occurs in a Gaussian model with a common temporal decay rate. Agents track a shared changing signal, with a cost for tracking error and disagreement. When agents keep using current local measurements, each needs only three numbers: its current reading, its reading at the pooling time, and the pooled average. These attain the same optimal performance as sharing all sensor histories through the pooling time while retaining current local observations. We quantify the value of keeping the old local reading and show that the pooling gain decays exponentially with age. The law extends to unequal sensor quality and independent signal components with different temporal rates. It also gives an explicit communication-price threshold, or equivalent latency threshold, beyond which no refreshing is optimal. Below the price threshold, the optimal periodic interval is unique. Periodic refreshing is optimal among all schedules independent of the sensor processes. Every result is proved within the letter.
\end{abstract}
\noindent\textbf{Keywords:} Decentralized control, sensor fusion, control system architecture.

\section{Introduction}
Sharing every sensor's data can lead to worse team decisions than acting locally. Collecting and distributing the data takes time, and a changing environment can make a broad view outdated before it is used. We identify the exact delay beyond which fresh local measurements outperform a complete delayed observation pool. Both decision rules are optimized for their available information. In an eight-sensor example, local operation has about $26\%$ lower team loss than decisions based on a pool one coherence time old, before any communication charge is included.

We consider agents tracking a common changing signal through noisy local sensors. Their shared objective penalizes tracking error and disagreement among their actions. The signal and sensor errors evolve as Gaussian processes with a common temporal decay rate. Agents can act on current local readings, use a delayed pool, or combine both. For the combined architecture, we prove that each agent needs just its current reading, its reading at the last pooling time, and the average of all sensors' readings at that time. Those three numbers achieve the same minimum team loss as giving each agent every sensor's history up to the pooling time, together with its current local observations.

The main results are:
\begin{enumerate}
\item \emph{Optimal local and pooled decisions.} We derive the optimal policies, posterior variances, and team losses, including the effect of the disagreement penalty (Proposition~\ref{prop:endpoints}).
\item \emph{An exact delay crossover.} We identify precisely when fresh local decisions have lower team loss than decisions based only on a complete delayed pool (Corollary~\ref{cor:crossover}).
\item \emph{Three numbers suffice for hybrid decisions.} We derive the optimal combined policy and show that its pooling gain decays exactly exponentially with age. Its posterior variance obeys a matching interpolation law (Theorem~\ref{thm:hybrid}).
\item \emph{The value of local storage.} We quantify the loss from discarding the old local reading. Keeping it strictly improves performance at every positive finite pool age (Proposition~\ref{prop:current}).
\item \emph{Unequal sensor quality.} A weighted pooled average gives the same exponential law when sensors have different error variances (Proposition~\ref{prop:hetero}).
\item \emph{Different temporal rates.} For independent signal components, the total pooling gain is a sum of exponentials weighted by their decision value (Corollary~\ref{cor:modes}).
\item \emph{When and how often to communicate.} We derive the communication-price and latency thresholds for choosing no refreshes. Below the price threshold, the unique optimal period is optimal among all schedules independent of the observations (Theorem~\ref{thm:refresh}).
\end{enumerate}
Each refresh supplies a finite total improvement while agents continue using fresh local observations. Once its price reaches that lifetime benefit, local operation is optimal.

Static team theory gives optimality conditions for agents with different observations \cite{radner1962team}, and common-information methods separate shared and private information \cite{afshari2017static}. Age of information \cite{yates2021aoi}, nonlinear age penalties \cite{sun2017update}, and remote estimation \cite{ornee2021ou} connect freshness to performance. Ballotta and coauthors study communication latency in distributed control \cite{ballotta2023latency} and its effect on the spatial locality of optimal proportional feedback \cite{ballotta2026role}.
...
\end{equation}
The disturbance represents temporally correlated local measurement error. The common rate in \eqref{eq:model} makes the sensor vector evolve by one scalar prediction factor.
...
\end{equation}
Its pool is $\tau$ time units old and its local information is current. Let $J_L$, $J_P$, $J_D(\tau)$, and $J_H(\tau)$ be the respective minimum expected losses. Section~III shows that the hybrid optimum needs only $Y_i(t)$, the retained sample $Y_i(s)$, and the latest pooled mean $\bar Y(s)$. The same value therefore applies when pooled information arrives only at selected refresh times. Table~\ref{tab:information} summarizes the data used by the policies, including a restricted-memory comparison in Section~\ref{subsec:memory}.
...
The next condition establishes optimality without restricting policies to linear rules.
...
\end{lemma}
\begin{proof}
For a feasible perturbation $w$, expansion of \eqref{eq:loss} gives the linear term
$2n^{-1}\sum_i\E[w_i((1+\kappa)u_i-\kappa\bar u-X)]$.
It vanishes for every $w$ exactly when \eqref{eq:normal} holds. The remaining term is
$n^{-1}\sum_i\E[w_i^2]+\kappa n^{-1}\sum_i\E[(w_i-\bar w)^2]$,
which is positive for every nonzero perturbation. This proves necessity, sufficiency, and uniqueness.
\end{proof}
Define
\begin{equation}
...
\end{theorem}
\begin{proof}
For $\tau=0$, \eqref{eq:hybridpolicy} is the pooled policy. Suppose $\tau>0$ and enlarge each agent's information to $\mathcal F(s)\vee\mathcal F_i(t)$. Set
\begin{equation}
...
We first justify using only the endpoint of each private innovation path. For $s<r<t$, let $Z_i(r)=Y_i(r)-e^{-\lambda(r-s)}Y_i(s)$ and
\begin{equation}
 c(r)=\frac{e^{-\lambda(t-r)}(1-e^{-2\lambda(r-s)})}
 {1-e^{-2\lambda(t-s)}}.\label{eq:bridge}
\end{equation}
A covariance calculation shows that every $Z_i(r)-c(r)Z_i$ is uncorrelated with the entire vector $Z$. These bridge variables are also independent of the old sensor history and of $\eta(t)$. Joint Gaussianity makes the bridges jointly independent of the variables that determine the current loss. Averaging the policies over the bridges preserves feasibility and cannot increase a convex loss. Hence private paths add no value beyond $Z_i$.

Condition on $\mathcal F(s)$. The first term in \eqref{eq:targetdecomp} is common knowledge. After subtracting it from every action, the residual team observes $Z_i$ and tracks $h\bar Z+\eta(t)$. Gaussian regression gives
\begin{align*}
 \E[h\bar Z+\eta(t)\mid Z_i]&=\frac{a}{a+b}Z_i,\\
 \E[k\bar Z\mid Z_i]&=\frac{kv}{a+b}Z_i.
\end{align*}
Since $(1+\kappa)(a+b)-\kappa v=d$, Lemma~\ref{lem:normal} proves that $kZ_i$ is the optimal correction. The resulting policy \eqref{eq:hybridpolicy} is feasible under \eqref{eq:hybrid}, so it also attains the lower bound from complete delayed sharing.
...
\end{proof}
The first term in \eqref{eq:hybridpolicy} predicts the signal from the pooled average. The second corrects it using the difference between the current local reading and its prediction from the stored reading. Evaluation needs the pooled average, the local reading stored at that pooling time, and the current reading. Earlier data add no decision value.
...
\end{equation}
This age budget is independent of $n$, $a$, $b$, and $\kappa$. Those parameters determine the amount $\Delta$ of value available to preserve. The value half-life is $\log 2/(2\lambda)$.
...
\begin{align*}
 ...
 \Cov(R_i,\bar R)=v(1-z).
\end{align*}
Conditioning on $M$ and applying Lemma~\ref{lem:normal} gives the unique optimal policy
\begin{equation}
 u_i=\sqrt z\frac avM+\frac{a(1-z)}{d-zv}R_i.\label{eq:currentpolicy}
\end{equation}
Here $d-zv>0$. Substitution into the conditional quadratic cost yields
\begin{equation}
...
\end{proof}
Keeping one local sample therefore improves the value of the same broadcast without additional communication.
...
\begin{equation}
 ...
 \quad h_0=\frac1n\sum_i C_i^{-1}.\label{eq:heterodefs}
\end{equation}
...
\begin{align}
 k_i&=\frac{a}{C_i(1-\kappa ah_0)},&
 J_L&=a-\frac{a^2h_0}{1-\kappa ah_0},\notag\\
...
\end{proposition}
\begin{proof}
Substitution of $u_i=k_iY_i$ into \eqref{eq:normal} gives $C_i k_i-\kappa a\bar k=a$, where $\bar k=n^{-1}\sum_i k_i$. Averaging $k_i=(a+\kappa a\bar k)/C_i$ solves for $\bar k$ and gives \eqref{eq:hetero}. The denominator is positive because $C_i>(1+\kappa)a$. At the optimum the quadratic term equals $a\bar k$, so $J_L=a-a\bar k$. Gaussian conditioning gives the displayed $P$ and $\mu_P$.

The residual $X(t)-\mu_P(t)$ is independent of the entire sensor process, since all temporal covariances share the same factor. Innovations over $[s,t]$ have covariance $(1-\rho^2)(a\mathbf1\mathbf1^\top+\operatorname{diag}(b_i))$. The bridge argument and conditional team decomposition in Theorem~\ref{thm:hybrid} therefore apply with these covariances and the coefficients $k_i$. They give \eqref{eq:heteropolicy} and the stated cost law.
\end{proof}
...
If summary $m$ has age $\tau_m$, its optimal policy is \eqref{eq:hybridpolicy} with the corresponding parameters. The total coordination value is
\begin{equation}
...
A communication service delivers each pooled vector after a fixed latency $\delta\ge0$. It supports the chosen transmission times without queueing. On reception, the pool's age resets to $\delta$. Agents retain local measurements from the associated sampling time. During transmission these samples can be kept in a timestamped buffer. Each vector costs $c>0$ in units of accumulated team loss. The refresh schedule is independent of all signal and disturbance processes. This includes deterministic schedules and independent randomized schedules.
...
To compare general schedules, set $g=\min\{0,\inf_{T>0}(c-B(T))/T\}$. Condition on any realization of an independent reception schedule. Each complete reception interval $T_j$ contributes at least $gT_j$ relative to fresh-local operation after assigning one communication cost to it. The last incomplete interval contributes at least $-c_{\rm crit}$, which bounds the gain over its entire lifetime. The initial interval before any reception has zero gain. Because $g\le0$, the total cost through time $R$ is at least
\[
 R(J_L^{\rm tot}+g)-c_{\rm crit}.
\]
Charging transmissions at generation preserves this lower bound because the number sent is at least the number received. Divide by $R$ and let $R\to\infty$. The bound holds for every schedule realization and hence for independent randomized schedules. Periodic operation at $T^*$, or no communication in the other case, attains it.
\end{proof}
...
\begin{figure}[t]
 ...
 \label{fig:decay}
\end{figure}
Figure~\ref{fig:decay} uses 120,000 independent stationary Gaussian sample pairs per marker. We also solve the Gaussian team normal equations directly from observation covariance matrices and compare the resulting policies and costs with the formulas. Across 126 systems, the maximum cost discrepancy is $5.3\times10^{-15}$.
...
\section{Conclusion}
Fresh local readings yield lower team loss than a complete delayed pool once its age exceeds an explicit threshold. An agent that also uses fresh local data can match the performance of complete delayed sharing using only three numbers: its current reading, its reading at the pooling time, and the pooled average. The improvement over local operation decays exponentially. Its finite lifetime value determines when communication is worth its price and how often to refresh. The pooling law also extends to unequal sensor quality and independent components with different temporal rates. The refresh result assumes fixed latency and schedules independent of the observations. Different signal and sensor-error rates within a component, or observation-dependent scheduling, require further analysis.

Simulation code, extended proofs, and proof verification reports will be posted at \url{https://github.com/anrgusc/TINA}.

\begin{samepage}
\section*{Acknowledgment}
OpenAI ChatGPT (Codex) \cite{chatgpt} was used to generate the abstract, text in Sections I--VII, and captions, and to assist with mathematical derivations and numerical code.
\end{samepage}
...
\bibitem{tina2026} S. Moeller and B. Krishnamachari, ``A theory of information architecture for networked decisions: Freshness, locality, and coordination,'' arXiv:2609.06351v1, 2026, longer version with broader coverage of information architectures.
...
\bibitem{chatgpt} OpenAI, \emph{ChatGPT (Codex)}, AI-assisted manuscript drafting and code generation, Sep. 11, 2026. [Online]. Available: \url{https://chatgpt.com/}
\end{thebibliography}
\end{document}
