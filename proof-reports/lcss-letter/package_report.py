...
package = root / 'output/TINA_Paper1_Lamport_Review.zip'
files = [
    'README.md', 'review_manifest.json', 'qa_report.py', 'package_report.py', 'audit_data.py', 'build_report.py', 'report_template.tex',
    ...
    'output/TINA_Paper1_Lamport_Report.tex',
    'output/TINA_Paper1_Lamport_Report.pdf',
]
for name in files:
...
with ZipFile(package, 'w', compression=ZIP_DEFLATED, compresslevel=9) as archive:
    for name in files:
        ...
        archive.write(root / name, 'TINA_Paper1_Lamport_Review/' + destination)
